...
:root{
  ...
  /* Schriften ausschliesslich aus dem System. Die Karte darf beim Anzeigen
     KEINE externe Anfrage ausloesen – ein Werkzeug, das Datenabfluesse
     sichtbar macht, kann seine Leserinnen nicht an einen Schriftdienst im
     Ausland melden. Wer IBM Plex installiert hat, sieht Plex; alle anderen
     sehen die Systemschrift. Beides ohne Netzwerkverkehr. */
  --text:"IBM Plex Sans",ui-sans-serif,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;
  --titel:"IBM Plex Sans",ui-sans-serif,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;
...
h1{font-family:var(--titel);font-weight:700;
  font-size:clamp(28px,5vw,40px);letter-spacing:-.01em;margin:8px 0 10px}
.meta{font-family:var(--mono);font-size:13px;color:var(--grau);
...
.fakt b{display:block;font-family:var(--mono);font-weight:600;font-size:17px}
...
.rechte h2{font-family:var(--titel);font-size:18px;margin-bottom:6px}
...
.rechte .frist{font-family:var(--mono);font-size:12.5px;color:var(--grau);margin-top:12px}
main{padding:8px 40px 8px}
main>h2{font-family:var(--titel);font-size:14px;letter-spacing:.1em;
...
.bearbeitung h3{font-family:var(--titel);font-size:21px;margin-bottom:4px}
...
.pfeil{align-self:center;font-family:var(--mono);color:var(--grau)}
...
.knoten-meta{font-family:var(--mono);font-size:11.5px;color:var(--grau)}
.knoten-tags{display:flex;flex-wrap:wrap;gap:4px;margin-top:3px}
.tag{font-family:var(--mono);font-size:10.5px;padding:2px 7px;
...
.b-fakten dt{font-family:var(--mono);font-size:12px;
...
footer{padding:22px 40px 30px;border-top:1px solid var(--linie);
  font-family:var(--mono);font-size:12px;color:var(--grau);
